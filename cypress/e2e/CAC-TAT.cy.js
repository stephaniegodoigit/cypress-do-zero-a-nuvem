describe('Central de Atendimento ao Cliente TAT', () => {
  ...
  it('preenche os campos obrigatórios e envia o formulário', () => {
    cy.clock() //comando para controlar o tempo
...
//  cy.get('button[type="submit"]').click() 
    ...
    cy.tick(3000) //avança o relógio virtual em 3 segundos

    cy.get('.success').should('not.be.visible')//verifica se a mensagem sumiu apos 3 segundos
  })
...
//  cy.get('button[type="submit"]').click() 
  ...
  it('faz uma requisição HTTP', () => {
    cy.request('https://cac-tat.s3.eu-central-1.amazonaws.com/index.html')
      .as('getRequest')//definindo um alias para a requisição
      .its('status')//acessando a propriedade status da requisição
      .should('be.equal', 200)
    cy.get('@getRequest')//chamando a requisição pelo alias
      .its('statusText')
      .should('be.equal', 'OK')
    cy.get('@getRequest')
      .its('body')
